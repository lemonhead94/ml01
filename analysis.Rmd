...
Above figure is showing the accuracy of each parameters for tuning.
...
Above is the confusion matrix of the neural network model. As we can see still the specificity of the model is quite low. 

```{r, echo=FALSE}
pred_raw <- predict(emp_net, emp_attrition_test, decision.values=TRUE, type = "prob")[, 2]

pred <- ROCR::prediction(pred_raw, emp_attrition_test$Attrition)
perf <- ROCR::performance(pred, measure="tpr", x.measure="fpr")

plot(perf, lwd=2, col="blue")
title("ROC Curve NN with all predictors")
abline(a=0, b=1)
auc_ROCR <- ROCR::performance(pred, measure="auc")
auc_ROCR <- auc_ROCR@y.values[[1]]
text(x = 0.9, y = 0.1, paste("auc: ",auc_ROCR), 
     cex = 1.6, col = "black")
```
