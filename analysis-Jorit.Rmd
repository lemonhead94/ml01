...
glm.attrition <- glm(Attrition ~ Age + BusinessTravel + 
    ...
    JobRole:OverTime + JobRole:WorkLifeBalance + NumCompaniesWorked:OverTime + 
    NumCompaniesWorked:StockOptionLevel + OverTime:RelationshipSatisfaction + 
    OverTime:StockOptionLevel + WorkLifeBalance:YearsSinceLastPromotion, data = emp_attrition, family = binomial)
...
```{r glm-plots, warning=FALSE, message=FALSE}
...
ggplot(emp_attrition, aes(OverTime, glm.attrition$fitted.values, color = OverTime)) +
...
ggplot(emp_attrition, aes(Age, glm.attrition$fitted.values, color = OverTime)) +
  geom_point(alpha = 0.15) +
